...
def create_startups_graph():
    ...
    people_concepts = people_concepts.sort_values(by='Score', ascending=False).groupby(by='SCIPER').head(5).reset_index(drop=True)

    # Unit nodes
    table_name = 'graph_piper.Nodes_N_Unit'
    fields = ['UnitID']
    conditions = {'UnitType': ['LABO', 'CENTRE', 'GROUPE', 'CHAIRE']}
    units = pd.DataFrame(db.find(table_name, fields=fields, conditions=conditions), columns=fields)
    unit_ids = list(units['UnitID'])

    # Person-unit edges
    table_name = 'graph_piper.Edges_N_Person_N_Unit'
    fields = ['SCIPER', 'UnitID', 'StartDate']
    conditions = {'SCIPER': founder_ids + professor_ids, 'UnitID': unit_ids, 'DurationMonths': {'>=': 12}}
    people_units = pd.DataFrame(db.find(table_name, fields=fields, conditions=conditions), columns=fields)
    people_units = people_units.sort_values(by=['SCIPER', 'StartDate']).drop_duplicates(subset=['SCIPER'], keep='last', ignore_index=True)
    people_units = people_units[['SCIPER', 'UnitID']]
    unit_ids = list(people_units['UnitID'].drop_duplicates())

    # Unit-concept edges
    table_name = 'graph.Edges_N_Unit_N_Concept_T_Research'
    fields = ['UnitID', 'PageID', 'Score']
    conditions = {'UnitID': unit_ids}
    units_concepts = pd.DataFrame(db.find(table_name, fields=fields, conditions=conditions), columns=fields)
    units_concepts = units_concepts.sort_values(by='Score', ascending=False).groupby(by='UnitID').head(5).reset_index(drop=True)

    ############################################################

    bc.log('Merging to obtain startup-concept edges through units...')

    startups_founders_units = pd.merge(startups_founders, people_units, how='inner', on='SCIPER')
    startups_professors_units = pd.merge(startups_professors, people_units, how='inner', on='SCIPER')

    # We give priority to the founders' unit and use the professors' as fallback
    startup_with_founder_ids = list(startups_founders_units['EPFLStartupID'].drop_duplicates())
    startups_professors_units = startups_professors_units[~startups_professors_units['EPFLStartupID'].isin(startup_with_founder_ids)]

    # Combine both in one DataFrame
    startups_units = pd.concat([startups_founders_units, startups_professors_units], ignore_index=True)
    startups_units_concepts = pd.merge(startups_units, units_concepts, how='inner', on='UnitID')
    startups_units_concepts = startups_units_concepts[['EPFLStartupID', 'PageID', 'Score']]

    ############################################################

    bc.log('Merging to obtain startup-concept edges through people...')
    ...
    # Combine both in one DataFrame
    startups_people_concepts = pd.concat([startups_founders_concepts, startups_professors_concepts], ignore_index=True)
    startups_people_concepts = startups_people_concepts.groupby(by=['EPFLStartupID', 'PageID']).aggregate({'Score': 'sum'}).reset_index()

    ############################################################

    bc.log('Concatenating to obtain final startup-concept edges...')

    # We give priority to the units' concepts and use the people' concepts as fallback
    startup_with_unit_ids = list(startups_units_concepts['EPFLStartupID'].drop_duplicates())
    startups_people_concepts = startups_people_concepts[~startups_people_concepts['EPFLStartupID'].isin(startup_with_unit_ids)]

    # Combine both in one DataFrame
    startups_concepts = pd.concat([startups_units_concepts, startups_people_concepts], ignore_index=True)
    ...
    seed_concept_ids = list(startups_concepts.loc[startups_concepts['EPFLStartupID'].isin(seed_startup_ids), 'PageID'].drop_duplicates())

    concept_ids = seed_concept_ids
    # concept_ids = list(set(seed_concept_ids) | set(seed_concept_ids) - set(excluded_concept_ids))
    ...
    proportion = 1    # keep 10% of all edges
    ...
    #
    # print(seed_startup_ids)
    # print(list(startups['EPFLStartupID']))
    # jsghdhsgdhghsg
