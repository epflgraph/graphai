...
def recompute_clusters_task(n_clusters):
    ...
    cluster_labels = cluster_and_reassign_outliers(embedding, n_clusters)
    unique_cluster_labels = sorted(list(set(cluster_labels.tolist())))
    result_dict = {
        label: [concept_index_to_id[i] for i in np.where(cluster_labels == label)[0]]
