...
class EmbeddingDBCachingManager(DBCachingManagerBase):
    def __init__(self, initialize_database=False):
        super().__init__(
            db_config=config['database'],
            cache_table='Embedding_Main',
            most_similar_table='Embedding_Most_Similar',
            schema=cache_schema,
            initialize_database=initialize_database
        )
    ...
    def init_db(self):
        ...
        self.db.execute_query(
            ...
            CREATE TABLE IF NOT EXISTS `{self.schema}`.`{self.cache_table}` (
              ...
              `embedding` VARCHAR(32768) DEFAULT NULL,
              `date_added` DATETIME DEFAULT NULL,
              PRIMARY KEY id_token (id_token)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
            """
