...
@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, retry_kwargs={"max_retries": 2},
             name='caching.translation_text_lookup_using_fingerprint', ignore_result=False)
...
@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, retry_kwargs={"max_retries": 2},
             name='caching.cache_lookup_translate_text', ignore_result=False)
def cache_lookup_translate_text_task(self, token, return_list=False):
    return cache_lookup_translate_text(token, return_list)


@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, retry_kwargs={"max_retries": 2},
             name='translation_gpu.translate_text', translation_obj=translation_models, ignore_result=False)
def translate_text_task(self, text, src, tgt, skip_sentence_segmentation=False, clean_and_segment=False):
    return translate_text(text, src, tgt, self.translation_obj, skip_sentence_segmentation, clean_and_segment)


@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, retry_kwargs={"max_retries": 2},
             name='translation_gpu.translate_text_callback', ignore_result=False)
...
@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, retry_kwargs={"max_retries": 2},
             name='translation_gpu.translate_text_return_list_callback', ignore_result=False)
...
@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, retry_kwargs={"max_retries": 2},
             name='translation_gpu.detect_text_language', ignore_result=False)
def detect_text_language_task(self, text):
    return detect_language_translation(text)


@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, retry_kwargs={"max_retries": 2},
             name='translation_gpu.clean_up_translation_object', model=translation_models, ignore_result=False)
