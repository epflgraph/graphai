...
plot_confs(comparative['pair_confs'], 'course_id', f'Pairs')
plot_confs(comparative['page_confs'], 'course_id', f'Pages')

for metric in ['p', 'r', 'f-score']:
    pairs_metric = np.mean([conf[metric] for conf in comparative['pair_confs']])
    pages_metric = np.mean([conf[metric] for conf in comparative['page_confs']])
