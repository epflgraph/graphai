...
with open('results/course-descriptions-text-to-results-results.json') as file:
...
for result in results:
    db_keyword_list = [db_result['keywords'] for db_result in result['db']]

    for db_keywords in db_keyword_list:
        t['n_keywords'] += 1

        [db_result] = [db_result for db_result in result['db'] if db_result['keywords'] == db_keywords]
        db_page_id = db_result['page_id']

        new_results = [new_result for new_result in result['new'] if new_result['keywords'] == db_keywords]
